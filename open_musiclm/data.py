...
class SoundDataset(Dataset):
    def __init__(
        ...
        exts="wav"
        files = [file for file in path.glob(f'**/instrument/*.{exts}')]
...
class MySoundDataset(Dataset):
    def __init__(
        ...
        ignore_file_set = set([f.split('/')[-1] for f in ignore_files])
        exts="wav"
        vocals_files = [file for file in path.glob(f'**/vocals/*.{exts}')]
        other_files = [file for file in path.glob(f'**/instrument/*.{exts}')]
        ...
        if len(vocals_files) != len(other_files):
          print("vocals_files",len(vocals_files),vocals_files)
          print("other_Files",len(other_files),other_files)

        if num_ignored > 0:
            print(f'skipped {num_ignored} ignored files')

        self.vocals_files = vocals_files
        ...
        num_outputs = len(self.target_sample_hz)
        '''jihoo'''
        max_length_seconds=(5.0,5.0,5.0)
        self.max_length_seconds = cast_tuple(max_length_seconds, num_outputs)
        self.max_length = tuple([int(s * hz) if exists(s) else None for s, hz in zip(self.max_length_seconds, self.target_sample_hz)])
    ...
    def __getitem__(self, idx):
        try:
            vocals_file = self.vocals_files[idx]
            other_file = self.other_files[idx]
            vocals_data, vocals_sample_hz = torchaudio.load(vocals_file)
            other_data, other_sample_hz = torchaudio.load(other_file)
        except:
            if self.ignore_load_errors:
                return self[torch.randint(0, len(self), (1,)).item()]
            else:
                raise Exception(f'error loading file {vocals_file} or {other_file}')
        vocals_semantic_wave,vocals_coarse_wave=self.my_process_audio(vocals_data,vocals_sample_hz)
        other_semantic_wave,_=self.my_process_audio(other_data,other_sample_hz)
        return vocals_semantic_wave,vocals_coarse_wave,other_semantic_wave
    ...
    def my_process_audio(self,data,sample_hz):
        ...
        return audio_for_wav2vec,audio_for_encodec
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.ignore_load_errors:
                ...
                raise Exception(f'error loading file {vocals_file} or {other_file}')
        vocals_semantic_wave,vocals_coarse_wave=self.my_process_audio(vocals_data,vocals_sample_hz)
        other_semantic_wave,other_coarse_wave=self.my_process_audio(other_data,other_sample_hz)
        return vocals_coarse_wave,other_semantic_wave,other_coarse_wave
