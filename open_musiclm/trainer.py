...
from .data import (PreprocessedDataset, SoundDataset, MySoundDataset,
                   get_dataloader,NewSoundDataset,
...
from .open_musiclm import (CoarseStage, FineStage, SemanticStage,SemcoarsetosemStage,
                           InstcoarseStage,
...
class SingleStageTrainer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if use_wandb_tracking:
            ...
            self.accelerator.init_trackers(wandb_name,config=hps)
        else:
            self.accelerator.init_trackers(f"{wandb_name}_{int(time.time() * 1000)}", config=hps)
    ...
    @contextmanager
    def wandb_tracker(self, run = None, hps = None):
        assert self.use_wandb_tracking, '`use_wandb_tracking` must be set to True on Trainer'

        hps = default(hps, self.tracker_hps)

        self.accelerator.init_trackers(self.stage_name, config = None)
    ...
    def train_step(self):
        ...
        if self.is_main and not (steps % self.save_model_every):
            ...
            if self.use_wandb_tracking:
              # current="/content/drive/MyDrive/my_code/mymusiclm/open-musiclm-main"
              model_path=wandb.run.dir +"/"+f'{self.wandb_name}.transformer.{steps}.pt'
              optim_path = wandb.run.dir +"/"+f'{self.wandb_name}.optimizer.{steps}.pt'
              scheduler_path = wandb.run.dir +"/"+f'{self.wandb_name}.scheduler.{steps}.pt'
              folder_path=wandb.run.dir
              if not os.path.exists(folder_path):
                  os.makedirs(folder_path)
            self.save(model_path, optim_path, scheduler_path)
            print("jihoo saving model",steps," ",model_path)
            self.print(f'{steps}: saving model to {str(self.results_folder)}')
...
@beartype_jit
class MySingleStageTrainer(nn.Module):
    """
    General trainer for any stage of MusicLM.
        semantic: requires audio_conditioner and wav2vec
        coarse: requires audio_conditioner, wav2vec, and neural_codec
    ...
    def __init__(
        ...
        *,
        ...
        # print("self.log_with", self.log_with )
        # self.log_with="wandb"
        ...
        self.stage = stage
        print("sel.fstage",stage)
        print("not in the semcoarsetosem")
        if stage == 'semcoarsetosem':
            print("in the semcoarsetosem")
            self.train_wrapper = SemcoarsetosemStage(
                  semcoarsetosem_transformer=transformer,
                  neural_codec=neural_codec,
                  wav2vec=wav2vec,
                  cross_entropy_loss_weights=default(cross_entropy_loss_weights, [0., 0., 1.])
            )
            ...
            #     self.ds_fields = ('clap_token_ids', 'semantic_token_ids')
            # else:
            self.ds_fields = ('vocals_raw_wave_for_wav2vec','vocals_raw_wave_for_acoustic', 'other_raw_wave_for_wav2vec')
            target_sample_hz = (wav2vec.target_sample_hz, neural_codec.sample_rate, wav2vec.target_sample_hz)
            print("target_sample_hz",target_sample_hz)
        ...
        self.ds = dataset
        if not exists(self.ds):
            # folder="/content/drive/MyDrive/data/5sec/train"
            assert exists(
                folder), 'folder must be passed in, if not passing in a custom dataset for text conditioned audio synthesis training'

            self.ds = MySoundDataset(
                folder,
                max_length_seconds=data_max_length_seconds,
                normalize=normalize,
                target_sample_hz=target_sample_hz,
        ...
        hps = asdict(self.model_config.global_cfg)
        if stage == 'semcoarsetosem':
            hps.update(asdict(self.model_config.semcoarsetosem_cfg))
            hps.update(asdict(self.training_config.semcoarsetosem_trainer_cfg))
    ...
    def train_step(self):
        ...
        for i in range(self.grad_accum_every):
            ...
            # print("next(self.dl_iter)",next(self.dl_iter))
            # print("self.ds_fields",self.ds_fields)
            data_kwargs = dict(zip(self.ds_fields, next(self.dl_iter)))
            # print(self.dl_iter)
            non_empty_batch = False
            while non_empty_batch is False:
        ...
        if not (steps % self.save_results_every):
            ...
            while non_empty_batch is False:
                ...
                pred_tokens = pred_tokens.detach().cpu().long()
                gt_tokens = gt_tokens.detach().cpu().long()
                # print("pred_tokens.shape,",pred_tokens.shape,pred_tokens)
                # print("gt_tokens.shape,",gt_tokens.shape,gt_tokens)
                valid_accuracy = (pred_tokens == gt_tokens).float().mean().item()
                self.print(f'{steps}: valid loss {valid_loss}, valid acc {valid_accuracy}')
...
@beartype_jit
class NewSingleStageTrainer(nn.Module):
    """
    General trainer for any stage of MusicLM.
        semantic: requires audio_conditioner and wav2vec
        coarse: requires audio_conditioner, wav2vec, and neural_codec
    ...
    def __init__(
        ...
        *,
        ...
        if stage == 'instcoarse':
            print("in the instcoarse")
            self.train_wrapper = InstcoarseStage(
                  instcoarse_transformer=transformer,
                  neural_codec=neural_codec,
                  wav2vec=wav2vec,
                  cross_entropy_loss_weights=default(cross_entropy_loss_weights, [0., 0., 1.])
            )
            ...
            #     self.ds_fields = ('clap_token_ids', 'semantic_token_ids')
            # else:
            self.ds_fields = ('vocals_raw_wave_for_acoustic', 'other_raw_wave_for_wav2vec','other_raw_wave_for_acoustic')
