...
class TokenConditionedTransformer(nn.Module):
    ...
    def forward(self,
        ...
        """
        # print("=================TokenConditionedTransformer=================")
        # print(all_token_ids[0].shape,all_token_ids[0])
        # print(all_token_ids[1].shape,all_token_ids[1])
...
@beartype_jit
def create_semcoarsetosem_transformer(
    dim=1024,
    depth=6,
    semantic_codebook_size=1024,
    acoustic_codebook_size=1024,
    ...
    **kwargs
    ...
    return TokenConditionedTransformer(token_sequences=[semantic_sequence, coarse_sequence,semantic_sequence], dim=dim, depth=depth, **kwargs)

@beartype_jit
def create_instcoarse_transformer(
    dim=1024,
    depth=6,
    semantic_codebook_size=1024,
    acoustic_codebook_size=1024,
    ...
    **kwargs
    ...
    return TokenConditionedTransformer(token_sequences=[coarse_sequence, semantic_sequence,coarse_sequence], dim=dim, depth=depth, **kwargs)
...
@beartype_jit
class SemcoarsetosemStage(nn.Module): # 결국, clap discrete
    def __init__(
        self,
        *,
        wav2vec=None,
        neural_codec: Optional[NeuralCodec] = None,
        semcoarsetosem_transformer: TokenConditionedTransformer,
        pad_id=-1,
        unique_consecutive=False,
        cross_entropy_loss_weights: List[float] = None,
        mask_prob=0.15
        ...
        self.wav2vec = wav2vec
        self.neural_codec = neural_codec
        num_semantic_tokens = semcoarsetosem_transformer.token_sequences[1].codebook_size
        if exists(wav2vec):
            assert self.wav2vec.codebook_size == num_semantic_tokens, f'num_semantic_tokens on CoarseTransformer must be set to {self.wav2vec.codebook_size}'
        self.num_coarse_quantizers = semcoarsetosem_transformer.token_sequences[-2].num_quantizers
    ...
    def generate(
        ...
        *,
        ...
        input_audio: Optional[torch.Tensor] = None,
        clap_token_ids: Optional[torch.Tensor] = None,
        vocals_semantic_token_ids: Optional[torch.Tensor] = None,
        vocals_coarse_token_ids: Optional[torch.Tensor] = None,
        filter_thres=0.9,
        temperature=1.,
        max_time_steps=30*25,
        include_eos_in_output=False,
        ...
        **kwargs
        ...
        #clap_tokens
        # semantic_token_ids = get_or_compute_semantic_token_ids(semantic_token_ids, conditioning_audio, self.wav2vec)

        sampled_tokens = self.transformer_wrapper.generate(
            conditioning_token_ids=[vocals_semantic_token_ids, vocals_coarse_token_ids],
            pred_token_ids=None,
            max_time_steps=max_time_steps,
            filter_thres=filter_thres,
            temperature=temperature,
    ...
    def forward(
        self,
        *,
        vocals_raw_wave_for_wav2vec=None,
        other_raw_wave_for_wav2vec=None,
        vocals_raw_wave_for_acoustic=None,
        clap_token_ids: Optional[torch.Tensor] = None,
        semantic_token_ids: Optional[torch.Tensor] = None,
        return_loss=False,
        **kwargs
        ...
        vocals_semantic_token_ids = get_or_compute_semantic_token_ids(semantic_token_ids=None, raw_audio=vocals_raw_wave_for_wav2vec, wav2vec=self.wav2vec)
        other_semantic_token_ids = get_or_compute_semantic_token_ids(semantic_token_ids=None, raw_audio=other_raw_wave_for_wav2vec, wav2vec=self.wav2vec)

        vocals_coarse_token_ids, _ = get_or_compute_acoustic_token_ids(
            coarse_token_ids=None,
            fine_token_ids=None,
            raw_audio=vocals_raw_wave_for_acoustic,
            neural_codec=self.neural_codec,
            num_coarse_quantizers=self.num_coarse_quantizers
        )
        return self.transformer_wrapper.forward(
            #vocals_clap_token_ids=torch.Size([2, 12, 1]),other_clap_token_ids=torch.Size([2, 12, 1])
            all_token_ids=[vocals_semantic_token_ids, vocals_coarse_token_ids,other_semantic_token_ids],
            return_loss=return_loss,
            **kwargs
        )
...
@beartype_jit
class InstcoarseStage(nn.Module): # 결국, clap discrete
    def __init__(
        self,
        *,
        wav2vec=None,
        neural_codec: Optional[NeuralCodec] = None,
        instcoarse_transformer: TokenConditionedTransformer,
        pad_id=-1,
        unique_consecutive=False,
        cross_entropy_loss_weights: List[float] = None,
        mask_prob=0.15
        ...
        self.wav2vec = wav2vec
        self.neural_codec = neural_codec
        self.num_coarse_quantizers = instcoarse_transformer.token_sequences[-1].num_quantizers

        self.transformer_wrapper = TokenConditionedTransformerWrapper(
            transformer=instcoarse_transformer,
            pad_id=pad_id,
            unique_consecutive=unique_consecutive,
            cross_entropy_loss_weights=cross_entropy_loss_weights,
            mask_prob=mask_prob
    ...
    def generate(
        ...
        *,
        ...
        input_audio: Optional[torch.Tensor] = None,
        clap_token_ids: Optional[torch.Tensor] = None,
        other_semantic_token_ids: Optional[torch.Tensor] = None,
        vocals_coarse_token_ids: Optional[torch.Tensor] = None,
        filter_thres=0.9,
        temperature=1.,
        max_time_steps=30*25,
        ...
        **kwargs
        ...
        sampled_tokens = self.transformer_wrapper.generate(
            conditioning_token_ids=[vocals_coarse_token_ids,other_semantic_token_ids],
            pred_token_ids=None,
            max_time_steps=max_time_steps,
            filter_thres=filter_thres,
            temperature=temperature,
    ...
    def forward(
        ...
        *,
        vocals_raw_wave_for_acoustic=None,
        other_raw_wave_for_wav2vec=None,
        other_raw_wave_for_acoustic=None,
        clap_token_ids: Optional[torch.Tensor] = None,
        semantic_token_ids: Optional[torch.Tensor] = None,
        return_loss=False,
        **kwargs
        ...
        other_coarse_token_ids, _ = get_or_compute_acoustic_token_ids(
            coarse_token_ids=None,
            fine_token_ids=None,
            raw_audio=other_raw_wave_for_acoustic,
            neural_codec=self.neural_codec,
            num_coarse_quantizers=self.num_coarse_quantizers
        )
        return self.transformer_wrapper.forward(
            #vocals_clap_token_ids=torch.Size([2, 12, 1]),other_clap_token_ids=torch.Size([2, 12, 1])
            all_token_ids=[vocals_coarse_token_ids, other_semantic_token_ids,other_coarse_token_ids],
            return_loss=return_loss,
            **kwargs
        )
