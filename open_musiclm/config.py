...
from .open_musiclm import (MusicLM, TokenConditionedTransformer,
                           create_coarse_transformer, create_fine_transformer,create_instcoarse_transformer,
                           create_semantic_transformer,create_semcoarsetosem_transformer)
from .trainer import ClapRVQTrainer, HfHubertKmeansTrainer, SingleStageTrainer,MySingleStageTrainer,NewSingleStageTrainer
...
class MyMusicLMModelConfig:
    ...
    coarse_cfg: CoarseConfig
    fine_cfg: FineConfig
    semcoarsetosem_cfg: SemcoarsetosemConfig
    global_cfg: GlobalConfig

@beartype_jit
@dataclass
class NewMusicLMModelConfig:
    clap_rvq_cfg: ClapRVQConfig
    hubert_kmeans_cfg: HubertKmeansConfig
    encodec_cfg: EncodecConfig
    semantic_cfg: SemanticConfig
    ...
    fine_cfg: FineConfig
    semcoarsetosem_cfg: SemcoarsetosemConfig
    instcoarse_cfg: InstcoarseConfig
    global_cfg: GlobalConfig
...
@beartype_jit
@dataclass
class MyMusicLMTrainingConfig:
    clap_rvq_trainer_cfg: ClapRVQTrainerConfig
    hubert_kmeans_trainer_cfg: HubertKmeansTrainerConfig
    semantic_trainer_cfg: SingleStageTrainerConfig
    coarse_trainer_cfg: SingleStageTrainerConfig
    fine_trainer_cfg: SingleStageTrainerConfig
    semcoarsetosem_trainer_cfg:SingleStageTrainerConfig
    data_preprocessor_cfg: DataPreprocessorConfig

@beartype_jit
@dataclass
class NewMusicLMTrainingConfig:
    clap_rvq_trainer_cfg: ClapRVQTrainerConfig
    hubert_kmeans_trainer_cfg: HubertKmeansTrainerConfig
    semantic_trainer_cfg: SingleStageTrainerConfig
    coarse_trainer_cfg: SingleStageTrainerConfig
    fine_trainer_cfg: SingleStageTrainerConfig
    semcoarsetosem_trainer_cfg:SingleStageTrainerConfig
    instcoarse_trainer_cfg:SingleStageTrainerConfig
    data_preprocessor_cfg: DataPreprocessorConfig
...
@beartype_jit
def my_load_model_config(config_path: str) -> MyMusicLMModelConfig:
    with open(config_path, 'r') as f:
        config = json.load(f)

    return MyMusicLMModelConfig(
        clap_rvq_cfg=ClapRVQConfig(**config['clap_rvq_cfg']),
        hubert_kmeans_cfg=HubertKmeansConfig(**config['hubert_kmeans_cfg']),
        encodec_cfg=EncodecConfig(**config['encodec_cfg']),
        semantic_cfg=SemanticConfig(**config['semantic_cfg']),
        coarse_cfg=CoarseConfig(**config['coarse_cfg']),
        fine_cfg=FineConfig(**config['fine_cfg']),
        semcoarsetosem_cfg=SemcoarsetosemConfig(**config['semcoarsetosem_cfg']),
        global_cfg=GlobalConfig(**config['global_cfg']),
    )
    
@beartype_jit
def new_load_model_config(config_path: str) -> NewMusicLMModelConfig:
    with open(config_path, 'r') as f:
        config = json.load(f)
    
    return NewMusicLMModelConfig(
        clap_rvq_cfg=ClapRVQConfig(**config['clap_rvq_cfg']),
        hubert_kmeans_cfg=HubertKmeansConfig(**config['hubert_kmeans_cfg']),
        encodec_cfg=EncodecConfig(**config['encodec_cfg']),
        semantic_cfg=SemanticConfig(**config['semantic_cfg']),
        ...
        fine_cfg=FineConfig(**config['fine_cfg']),
        semcoarsetosem_cfg=SemcoarsetosemConfig(**config['semcoarsetosem_cfg']),
        instcoarse_cfg=InstcoarseConfig(**config['instcoarse_cfg']),
        global_cfg=GlobalConfig(**config['global_cfg']),
    )
...
@beartype_jit
def my_load_training_config(config_path: str) -> MyMusicLMTrainingConfig:
    with open(config_path, 'r') as f:
        config = json.load(f)

    return MyMusicLMTrainingConfig(
        clap_rvq_trainer_cfg=ClapRVQTrainerConfig(**config['clap_rvq_trainer_cfg']),
        hubert_kmeans_trainer_cfg=HubertKmeansTrainerConfig(**config['hubert_kmeans_trainer_cfg']),
        semantic_trainer_cfg=SingleStageTrainerConfig(**config['semantic_trainer_cfg']),
        coarse_trainer_cfg=SingleStageTrainerConfig(**config['coarse_trainer_cfg']),
        fine_trainer_cfg=SingleStageTrainerConfig(**config['fine_trainer_cfg']),
        semcoarsetosem_trainer_cfg=SingleStageTrainerConfig(**config['semcoarsetosem_trainer_cfg']),
        data_preprocessor_cfg=DataPreprocessorConfig(**config['data_preprocessor_cfg']),
    )

@beartype_jit
def new_load_training_config(config_path: str) -> NewMusicLMTrainingConfig:
    with open(config_path, 'r') as f:
        config = json.load(f)

    return NewMusicLMTrainingConfig(
        clap_rvq_trainer_cfg=ClapRVQTrainerConfig(**config['clap_rvq_trainer_cfg']),
        hubert_kmeans_trainer_cfg=HubertKmeansTrainerConfig(**config['hubert_kmeans_trainer_cfg']),
        semantic_trainer_cfg=SingleStageTrainerConfig(**config['semantic_trainer_cfg']),
        coarse_trainer_cfg=SingleStageTrainerConfig(**config['coarse_trainer_cfg']),
        fine_trainer_cfg=SingleStageTrainerConfig(**config['fine_trainer_cfg']),
        semcoarsetosem_trainer_cfg=SingleStageTrainerConfig(**config['semcoarsetosem_trainer_cfg']),
        instcoarse_trainer_cfg=SingleStageTrainerConfig(**config['instcoarse_trainer_cfg']),
        data_preprocessor_cfg=DataPreprocessorConfig(**config['data_preprocessor_cfg']),
    )
...
@beartype_jit
def create_semcoarsetosem_transformer_from_config(
    model_config: MyMusicLMModelConfig,
    checkpoint_path: Optional[str],
    device,
    **kwargs,
) -> TokenConditionedTransformer:
    transformer = create_semcoarsetosem_transformer(
        **asdict(model_config.semcoarsetosem_cfg),
        semantic_codebook_size=model_config.hubert_kmeans_cfg.codebook_size,
        acoustic_codebook_size=model_config.encodec_cfg.codebook_size,
        num_coarse_quantizers=model_config.global_cfg.num_coarse_quantizers,
...
@beartype_jit
def create_instcoarse_transformer_from_config(
    model_config: NewMusicLMModelConfig,
    checkpoint_path: Optional[str],
    device,
    **kwargs,
) -> TokenConditionedTransformer:
    transformer = create_instcoarse_transformer(
        **asdict(model_config.instcoarse_cfg),
        semantic_codebook_size=model_config.hubert_kmeans_cfg.codebook_size,
        acoustic_codebook_size=model_config.encodec_cfg.codebook_size,
        num_coarse_quantizers=model_config.global_cfg.num_coarse_quantizers,
...
def create_single_stage_trainer_from_config(
    ...
    **kwargs,
    ...
    return trainer
@beartype_jit
def my_create_single_stage_trainer_from_config(
    model_config: MyMusicLMModelConfig,
    training_config: MyMusicLMTrainingConfig,
    stage: Literal['semantic', 'coarse', 'fine','semcoarsetosem'],
    results_folder: str,
    transformer: TokenConditionedTransformer,
    clap: Optional[ClapQuantized]=None,
    wav2vec: Optional[HfHubertWithKmeans]=None,
    ...
    config_paths: Optional[List[str]] = None,
    **kwargs,
) -> MySingleStageTrainer:

    semantic_audio_length_seconds = model_config.global_cfg.semantic_audio_length_seconds
    coarse_audio_length_seconds = model_config.global_cfg.coarse_audio_length_seconds

    # print("my_create_single_stage_trainer_from_config",stage)
    if stage == 'semcoarsetosem':

        trainer_cfg = training_config.semcoarsetosem_trainer_cfg
        data_max_length_seconds = (semantic_audio_length_seconds, semantic_audio_length_seconds, semantic_audio_length_seconds)


    trainer = MySingleStageTrainer(
        model_config=model_config,
        training_config=training_config,
        transformer=transformer,
        audio_conditioner=clap,
...
@beartype_jit
def new_create_single_stage_trainer_from_config(
    model_config: NewMusicLMModelConfig,
    training_config: NewMusicLMTrainingConfig,
    stage: Literal['semantic', 'coarse', 'fine','semcoarsetosem','instcoarse'],
    results_folder: str,
    transformer: TokenConditionedTransformer,
    clap: Optional[ClapQuantized]=None,
    wav2vec: Optional[HfHubertWithKmeans]=None,
    ...
    **kwargs,
    ...
    if stage == 'instcoarse':
        ...
        trainer_cfg = training_config.instcoarse_trainer_cfg
        data_max_length_seconds = (semantic_audio_length_seconds, semantic_audio_length_seconds, semantic_audio_length_seconds)

    trainer = NewSingleStageTrainer(
        model_config=model_config,
        training_config=training_config,
        transformer=transformer,
        audio_conditioner=clap,
